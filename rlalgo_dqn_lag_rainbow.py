...
class RainbowDQNLag():
    def __init__(
        ...
        self.task_q = RainbowQDiscreteSingleAction(obs_dim=obs_dim, hidden_dim=hidden_dim, action_dim=action_dim, atom_size=atom_size, support=self.support).to(device)
        self.tar_task_q = RainbowQDiscreteSingleAction(obs_dim=obs_dim, hidden_dim=hidden_dim, action_dim=action_dim, atom_size=atom_size, support=self.support).to(device)

        self.safe_q = rainbow

        for tar_param,param in zip(self.tar_task_q.parameters(),self.task_q.parameters()):
            tar_param.data.copy_(param.data)

        self.q_optim = optim.Adam(self.task_q.parameters(),lr=q_lr)
    ...
    def update(self, replay_buffer, batch_size, reward_scale=10., gamma=0.99, prior_eps=1e-6):
        ...
        with torch.no_grad():
            ...
            next_action = self.task_q.forward(obs=next_obs).argmax(dim=-1)
            next_dist = self.tar_task_q.get_dist(obs=next_obs)
        ...
        dist = self.task_q.get_dist(obs=obs)
        ...
        self.task_q.reset_noise()
        self.tar_task_q.reset_noise()


    def save_model(self, path):
        torch.save(self.task_q.state_dict(), path+'_critic')

    def load_model(self, path):
        self.task_q.load_state_dict(torch.load(path+'_critic'))
...
def train_or_test(train_or_test):
    ...
    if train_or_test == 'train':
        ...
        for step in range(1,max_timeframe+1):
            ...
            action = agent.task_q.get_action(obs=obs,epsilon=epsilon,deterministic=deterministic)
