...
class TD3_DeterministicContinuous():
    def __init__(
        ...
        if is_single_or_multi_out == 'single_out':
            ...
            self.policy = TD3_DeterministicContinuousPolicyMultiActionSingleOutLayer(device=device, obs_dim=obs_dim, action_dim=action_dim, hidden_dim=hidden_dim, action_range=action_range).to(device)
            self.tar_policy = TD3_DeterministicContinuousPolicyMultiActionSingleOutLayer(device=device, obs_dim=obs_dim, action_dim=action_dim, hidden_dim=hidden_dim, action_range=action_range).to(device)
    ...
    def update(self,replay_buffer, batch_size, gamma=0.99, soft_tau=1e-2, reward_scale=10.):
        self.update_cnt += 1
        ...
        if self.is_single_or_multi_out == 'single_out':
            # update q
            new_next_action = self.tar_policy.evaluate(obs=next_obs, is_tar=True)
            ...
            if self.update_cnt % self.policy_delay_update_interval == 0:
                new_action = self.policy.evaluate(obs=obs, is_tar=False)
...
class TD3_GaussianContinuous():
    def __init__(
        ...
        if is_single_or_multi_out == 'single_out':
            ...
            self.tar_critic1 = TD3_QContinuousMultiActionSingleOutLayer(obs_dim=obs_dim, action_dim=action_dim, hidden_dim=hidden_dim).to(device)
            self.tar_critic2 = TD3_QContinuousMultiActionSingleOutLayer(obs_dim=obs_dim, action_dim=action_dim, hidden_dim=hidden_dim).to(device)
            self.policy = TD3_GaussianContinuousPolicyMultiActionSingleOutLayer(device=device, obs_dim=obs_dim, action_dim=action_dim, hidden_dim=hidden_dim, action_range=action_range, log_std_min=log_std_min, log_std_max=log_std_max).to(device)
            self.tar_policy = TD3_GaussianContinuousPolicyMultiActionSingleOutLayer(device=device, obs_dim=obs_dim, action_dim=action_dim, hidden_dim=hidden_dim, action_range=action_range, log_std_min=log_std_min, log_std_max=log_std_max).to(device)
