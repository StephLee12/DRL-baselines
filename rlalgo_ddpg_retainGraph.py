...
class DDPG_DeterministicContinuous():
    ...
    def update(self,replay_buffer,batch_size,noise_scale,reward_scale=10.0, gamma=0.99, soft_tau=1e-2, tar_update_delay=3):
        ...
        if self.is_single_or_multi_out == 'single_out':
            # cal q loss 
            new_next_action = self.tar_policy.evaluate(obs=next_obs,noise_std=noise_scale)
            ...
            # cal policy loss 
            new_action = self.policy.evaluate(obs=obs,noise_std=noise_scale)
...
class DDPG_GaussianContinuous():
    ...
    def update(self, replay_buffer, batch_size, noise_std, reward_scale=10.0, gamma=0.99, soft_tau=1e-2, tar_update_delay=3):
        ...
        if self.is_single_or_multi_out == 'single_out':
            ...
            new_next_action = self.tar_policy.evaluate(obs=next_obs,noise=noise_std) 
            tar_next_q_lst = self.tar_critic.evalaute(obs=next_obs,action=new_next_action)
            tar_q_lst = [reward + (1-done)*gamma*tar_next_q for tar_next_q in tar_next_q_lst]
            ...
            new_action = self.policy.evaluate(obs=obs,noise=noise_std)
            new_q_lst = self.critic(obs=obs,action=new_action)
            policy_loss = 0
            for new_q in new_q_lst:
                policy_loss += -new_q.mean()
...
def train_or_test(train_or_test):
    ...
    if train_or_test == 'train':
        ...
        for step in range(1, max_timeframe+1):
            action = agent.policy.get_action(obs=obs, deterministic=deterministic, noise_std=noise_std)
            next_obs, reward, dw, tr, info = env.step(action)
            done = (dw or tr)
            replay_buffer.push(obs, action, reward, next_obs, done, dw)
            score += reward 
            if done: 
                obs, _ = env.reset()
