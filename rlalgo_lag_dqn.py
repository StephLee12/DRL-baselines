...
class DQN():
    def __init__(
        ...
        if is_single_multi_out == 'single_out':
            ...
            self.safe_q = QDiscreteSingleAction(obs_dim=obs_dim, hidden_dim=hidden_dim, action_dim=action_dim).to(self.device)
            self.tar_safe_q = QDiscreteSingleAction(obs_dim=obs_dim, hidden_dim=hidden_dim, action_dim=action_dim).to(self.device)
        ...
        self._target_hard_update(tar_net=self.tar_q, net=self.q)
        self._target_hard_update(tar_net=self.tar_safe_q, net=self.safe_q)

        self.q_optim = optim.Adam(self.q.parameters(), lr=q_lr)
        self.safe_q_optim = optim.Adam(self.safe_q.parameters(), lr=q_lr)
        
        self.lamda = torch.tensor(init_lamda, requires_grad=True, devide=device)
    ...
    def update(self, replay_buffer, batch_size, reward_scale=10., gamma=0.99, update_interval=32, soft_tau=0.005, update_manner='hard'):
        ...
        if self.is_single_multi_out == 'single_out':
            ...
            safe_q = self.safe_q.forward(obs=obs)
            safe_q_a = safe_q.gather(dim=1, index=action.unsqueeze(-1).long())
            min_next_safe_q_a = self.tar_safe_q.forward(obs=next_obs).min(dim=-1)[0].unsqueeze(-1)
            
            tar_safe_q_a = cost + gamma * (1-dw) * min_next_safe_q_a
            
            safe_q_loss = q_loss_func(safe_q_a, tar_safe_q_a.detach())
            
            if self.update_cnt % self.lamda_update_interval == 0:
                self.log_lamda = F.softplus(self.lamda)
                
                lamda_loss = (self.log_lamda * safe_q_a.detach()).sum(dim=-1)
        ...
        self.safe_q_optim.zero_grad()
        safe_q_loss.backward()
        self.safe_q_optim.step()
        ...
        if self.is_single_multi_out == 'single_out':
            if update_manner == 'hard':
                ...
                self._soft_hard_update(tar_net=self.tar_q, net=self.q, soft_tau=soft_tau)
                self._soft_hard_update(tar_net=self.tar_safe_q, net=self.safe_q, soft_tau=soft_tau)
